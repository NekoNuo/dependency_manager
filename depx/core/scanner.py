...
from typing import List, Generator, Dict, Type, Optional
...
class ProjectScanner:
    ...
    def scan_directory(self, root_path: Path, max_depth: int = 5, 
                      parallel: bool = True) -> List[ProjectInfo]:
        """
        扫描指定目录，发现所有项目
    ...
    def _find_project_candidates(self, root_path: Path, max_depth: int) -> Generator[Path, None, None]:
        """
        查找潜在的项目目录
        ...
        Yields:
            潜在的项目目录路径
        """
        def _scan_recursive(current_path: Path, current_depth: int) -> Generator[Path, None, None]:
    ...
    def _parse_projects_sequential(self, project_paths: List[Path]) -> List[ProjectInfo]:
        """
        顺序解析项目
    ...
    def _parse_single_project(self, project_path: Path) -> Optional[ProjectInfo]:
        ...
        for parser in self._parsers.values():
            if parser.can_parse(project_path):
                logger.debug(f"使用 {parser.project_type.value} 解析器解析: {project_path}")
