...
from ..parsers.base import ProjectInfo, DependencyInfo, ProjectType, PackageManagerType
...
class DependencyCleaner:
    ...
    def create_cleanup_plan(self, projects: List[ProjectInfo], 
                          cleanup_types: List[str] = None) -> CleanupPlan:
        """
        ...
        for project in projects:
            if 'dev' in cleanup_types:
                plan.project_dependencies.extend(
                    self._find_dev_dependencies(project)
                )
            
            if 'large' in cleanup_types:
                plan.project_dependencies.extend(
                    self._find_large_dependencies(project)
                )
    ...
    def clean_project_dependencies(self, project: ProjectInfo, 
                                 dependency_types: List[str] = None) -> CleanupResult:
        """
    ...
    def clean_global_caches(self, package_managers: List[PackageManagerType] = None) -> CleanupResult:
        """
    ...
    def _find_large_dependencies(self, project: ProjectInfo, 
                               threshold: int = 50 * 1024 * 1024) -> List[Dict[str, Any]]:
    ...
    def _clean_project_dependency(self, item: Dict[str, Any]) -> bool:
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would clean: {item['name']} ({format_size(item['size'])})")
            return True
    ...
    def _clean_global_cache(self, item: Dict[str, Any]) -> bool:
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would clean cache: {item['name']} ({format_size(item['size'])})")
            return True
    ...
    def _clean_nodejs_dependencies(self, project: ProjectInfo, 
                                 dependency_types: List[str]) -> CleanupResult:
            ...
            if node_modules.exists():
                if self.dry_run:
                    ...
                    try:
                        ...
                        result.cleaned_items.append("node_modules")
                        result.freed_space = project.total_size_bytes
                        logger.info(f"Removed node_modules: {format_size(project.total_size_bytes)}")
                    except Exception as e:
    ...
    def _clean_python_dependencies(self, project: ProjectInfo, 
                                 dependency_types: List[str]) -> CleanupResult:
                ...
                if venv_path.exists():
                    if self.dry_run:
                        ...
                        try:
                            ...
                            result.cleaned_items.append(venv_dir)
                            result.freed_space = project.total_size_bytes
                            logger.info(f"Removed {venv_dir}: {format_size(project.total_size_bytes)}")
                        except Exception as e:
