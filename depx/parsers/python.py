...
class PythonParser(BaseParser):
    ...
    def config_files(self) -> List[str]:
        return ["requirements.txt", "setup.py", "pyproject.toml", "Pipfile", "environment.yml"]
    ...
    def parse_project(self, project_path: Path) -> Optional[ProjectInfo]:
        ...
        for config_name in ["pyproject.toml", "setup.py", "requirements.txt", "Pipfile"]:
    ...
    def get_dependencies(self, project_info: ProjectInfo) -> List[DependencyInfo]:
        ...
        for req_file in ["requirements-dev.txt", "dev-requirements.txt", "test-requirements.txt"]:
            req_path = project_info.path / req_file
            if req_path.exists():
                dev_deps = self._parse_requirements_txt(req_path, DependencyType.DEVELOPMENT)
    ...
    def _parse_requirements_txt(self, file_path: Path, dep_type: DependencyType = DependencyType.PRODUCTION) -> List[DependencyInfo]:
    ...
    def _parse_pyproject_toml(self, file_path: Path) -> List[DependencyInfo]:
        ...
        try:
            ...
            for group_name, deps in optional_deps.items():
                dep_type = DependencyType.DEVELOPMENT if "dev" in group_name.lower() else DependencyType.OPTIONAL
